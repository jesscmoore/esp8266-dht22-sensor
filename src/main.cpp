...
#include <DHT.h>
#include "../config.h"
#define DHTPIN D2 // Specify full pin name with NodeMCU v3 Lolin. Not number only.
...
int ICACHE_FLASH_ATTR read_sensors(byte bus_idx, float &temp, float &humidity)
{
	...
	if (currentMillis - previousMillis[bus_idx] >= interval)
	{
		...
		if (isnan(temp) || temp == 85.0 || temp == (-127.0))
		{
			...
			if (previousMillis[bus_idx] < 0)
				previousMillis[bus_idx] = 0;
			return MEASURED_FAILED;
		}
		else
		{
...
#ifdef SENSOR_DHT22 // Initialise DHT22 if sensor is DHT22
	dht.begin();
#endif
	Serial.println("");
